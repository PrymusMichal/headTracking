...
namespace headTracking
{
    ...
    public static class Utility
    {
        ...
        public static double[] midpoint(Point2d p1, Point2d p2)
        {
            double[] points = new double[2];
            points[0]= (p1.X + p2.X) / 2;
            points[1] = (p1.Y + p2.Y) / 2;
            return points;
        }

        public static double euclideanDistance(double leftx, double lefty, double rightx, double righty)
        {
            return Math.Sqrt(Math.Abs(leftx - rightx) * 2 + Math.Abs(lefty - righty) * 2);
        }
    }
}